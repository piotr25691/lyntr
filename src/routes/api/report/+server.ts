...
export const POST: RequestHandler = async ({ request, cookies }) => {
	...
	try {
		...
		*/
		...
		return json({ message: 'To report messages go here: https://www.youtube.com/watch?v=dQw4w9WgXcQ' }, { status: 200 });
	} catch (error) {
		console.error('Error processing report:', error);
		return json({ error: 'To report messages go here: https://www.youtube.com/watch?v=dQw4w9WgXcQ' }, { status: 500 });
	}
};
